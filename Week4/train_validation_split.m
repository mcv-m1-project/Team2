function train_validation_split( dataPath, signalRepetitions )
...
%         filePath = strcat(data(file,1).folder,'\',data(file,1).name);
		...
		validationPath = [dataPath '\..\validation'];
		mkdir(validationPath);
		mkdir([validationPath '\gt']);
		mkdir([validationPath '\mask']);
		
        if validationSigns + fileSigns <= round(signalRepetitions * 0.3)
            movefile([dataPath '\' fileId '.jpg'],validationPath);
			movefile([dataPath '\gt\gt.' fileId '.txt'],[validationPath '\gt']);
			movefile([dataPath '\mask\mask.' fileId '.png'],[validationPath '\mask']);
