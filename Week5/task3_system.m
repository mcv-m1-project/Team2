function [out_mask, windowCandidates] = task3_system(image, params)
    ...
    mask_color = task3_colorSegmentation(image, params.cs);
    mask_mo = mask_color;
    windowCandidates = [];
...
%     mask_mo = task3_morphologicalOperators(mask_color, params.mo);


    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%% Select candidate windows %%%%%%%%%
%     windowCandidates = task3_connectedComponents(mask_mo, params.cc);


    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%% Apply Hough detection %%%%%%%%%%%%
%     windowCandidates = task3_houghDetection(image, mask_mo, windowCandidates, params.hd);
...
function out_mask = task3_colorSegmentation(image, params_cs)
    ...
    for p = 1:size(out_mask, 1)
        hist_i = pixels(p,1);
        hist_j = pixels(p,2);
        out_mask(p) = (red_hist(hist_i, hist_j) > th) |...
                      (blue_hist(hist_i, hist_j) > th) |...
                      (rb_hist(hist_i, hist_j) > th);
    end
...
function out_mask = task3_morphologicalOperators(mask, params_mo)
    ...
    signals_size = params_mo.signals_size;
...
function windowCandidates = task3_houghDetection(image, mask, windowCandidates, params_hd)
    ...
    grdthres = params_hd.grdthres;
    fltr4LM_R = params_hd.fltr4LM_R;
