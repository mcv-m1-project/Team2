function [hist_g1,hist_g2,hist_g3] = jointHistogramsByGroupHSV(trainSet, signals, numBins)
...
for idx = 1:size(trainSet,2) 
    ...
    if (isempty(im_index) == 0)
        ...
        for jdx=1:size(im_index)
            ...
            ch_h = im_test_signal(:,:,1);
            ch_s = im_test_signal(:,:,2);
                      
            h = hist3(double([ch_h(:) ch_s(:)]), [numBins numBins]);
            % [m idx] = max(h(:));
            % h(idx) = 0;
