function [ precisionMat, recallMat, FMat, thMat ] = backProjectionTrainHSV( dirTrainDataSet, trainSet, signals)
%BACKPROJECTIONTRAIN
...
for bin=1:length(numBins)
    bin
    [hist_g1,hist_g2,hist_g3] = jointHistogramsByGroupHSV(trainSet, signals, numBins(bin));
    ...
    for th=1:length(th_g1)
        ...
        for image=1:length(trainSet)
            ...
            [mask] = maskCalculationHSV(im_orig, hist_g1,hist_g2,hist_g3, th_g1(th), th_g2(th),th_g3(th));
